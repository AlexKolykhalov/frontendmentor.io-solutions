create or replace function api.get_initial_data() returns jsonb as
$$
declare
...
begin
  ...
  select jsonb_agg(jsonb_build_object('id', board_id, 'name', name, 'columns', '[]'::jsonb) order by id)
  into _boardsList
  from api.boards
  where user_id = _userID;

  return jsonb_build_object(
    'boardData', coalesce(_boardData, '{"id": "", "name": "Empty board", "columns": []}'::jsonb),
    'boardsList', coalesce(_boardsList, '[]'::jsonb),
