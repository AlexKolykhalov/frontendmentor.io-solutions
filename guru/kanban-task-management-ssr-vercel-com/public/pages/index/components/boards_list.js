...
import { insert }         from "./_helpers.js";
...
export class BoardsList {
  ...
  static template(props) {
    ...
    const boardsListItem = 
	  props.map((board, index) => BoardsListItem.template(
	    { id: board.id, title: board.name, selected: index === 0, locked: index === 1 },
	    { SSR: true }
	  ))
	  .join("");
  ...
  static handleEvents(component) {
	...
	BoardsListItem.init({
	  ...
	  title:    data.name,
	  selected: true,
	  locked:   Role.getRole() === roles.ANONYMOUS
	})
      );
